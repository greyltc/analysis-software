...
# this tests different fitting algorithms
...
def optimizeThis (x, I0, Iph, Rs, Rsh, n):
    ...
    return np.real_if_close((Rs*(I0*Rsh + Iph*Rsh - x) - thermalVoltage*n*(Rs + Rsh)*lambertw(I0*Rs*Rsh*np.exp((Rs*(I0*Rsh + Iph*Rsh - x) + x*(Rs + Rsh))/(thermalVoltage*n*(Rs + Rsh)))/(thermalVoltage*n*(Rs + Rsh)),tol=lambertWTol))/(Rs*(Rs + Rsh)))
...
fitReturn = optimize.curve_fit(optimizeThis, v, i, p0=guess, jac ='cs', xtol=0, ftol=1e-20, x_scale="jac", method=myMethod, verbose=2,max_nfev=1200000)

fitParams = fitReturn[0]
sumSqErr = sum([(optimizeThis(x, *fitParams)-y)**2 for x,y in zip(v,i)])

print(fitReturn)
print("The sum of the square of the errors is:")
print(sumSqErr)
