...
class MainWindow(QMainWindow):
    ...
    def __init__(self, analyzer):
        ...
        self.bounds["I0"][0] = float(I0_lb_string)
        self.bounds["Iph"][0] = float(Iph_lb_string)
        self.bounds["Rs"][0] = float(Rs_lb_string)
        self.bounds["Rsh"][0] = float(Rsh_lb_string)
        self.bounds["n"][0] = float(n_lb_string)

        self.bounds["I0"][1] = float(I0_ub_string)
        self.bounds["Iph"][1] = float(Iph_ub_string)
        self.bounds["Rs"][1] = float(Rs_ub_string)
        self.bounds["Rsh"][1] = float(Rsh_ub_string)
        self.bounds["n"][1] = float(n_ub_string)
    ...
    def rowGraph(self, row):
        ...
        if ss_voltage:
            plt.annotate(abs(thisGraphData["ssVocValue"]).__format__("0.4f") + " V", xy=(abs(thisGraphData["ssVocValue"]), 0), xytext=(40, 20), textcoords="offset points", ha="right", va="bottom", bbox=dict(boxstyle="round,pad=0.5", fc="yellow", alpha=0.5), arrowprops=dict(arrowstyle="->", connectionstyle="arc3,rad=0"))

        if ss_current:
            plt.annotate(abs(float(1000 * thisGraphData["ssJscValue"])).__format__("0.4f") + " mA/cm^2", xy=(0, abs(1000 * thisGraphData["ssJscValue"])), xytext=(40, 20), textcoords="offset points", ha="right", va="bottom", bbox=dict(boxstyle="round,pad=0.5", fc="yellow", alpha=0.5), arrowprops=dict(arrowstyle="->", connectionstyle="arc3,rad=0"))

        if ss_power:
            plt.annotate(abs(float(1000 * thisGraphData["ssJmpp"] * thisGraphData["ssVmpp"])).__format__("0.4f") + "mW/cm^2 @(" + abs(float(thisGraphData["ssVmpp"])).__format__("0.4f") + "," + abs(float(thisGraphData["ssJmpp"])).__format__("0.4f") + ")", xy=(abs(thisGraphData["ssVmpp"]), abs(1000 * thisGraphData["ssJmpp"])), xytext=(80, 40), textcoords="offset points", ha="right", va="bottom", bbox=dict(boxstyle="round,pad=0.5", fc="yellow", alpha=0.5), arrowprops=dict(arrowstyle="->", connectionstyle="arc3,rad=0"))
