...
class ivAnalyzer:
    ...
    def _compute_u(p, D, dydx, dx, dx1, n):
        ...
        if p is None or p < 1:
            Q = scipy.sparse.spdiags([dx1[: n - 2], -(dx1[: n - 2] + dx1[1 : n - 1]), dx1[1 : n - 1]], [0, -1, -2], n, n - 2)
            QDQ = Q.T * D * Q
            if p is None or p < 0:
                # Estimate p
                p = 1.0 / (1.0 + QDQ.diagonal().sum() / (100.0 * R.diagonal().sum() ** 2))
        ...
        if "homebrew" in mode:
            ...
            if n == 2:  # % straight line
                ...
                if p < 1:
                    ...
                    ai = (y - (6 * (1 - p) * D * np.diff(np.vstack([zrs, np.diff(np.vstack([zrs, u, zrs]), axis=0) * dx1, zrs]), axis=0)).T).T
                else:
                    ai = y.reshape(n, -1)
